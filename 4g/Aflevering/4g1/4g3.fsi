...
/// The overall impact to performance would also be detrimental, wasting cycles and memory to an extravagant extent.  
/// Finally the use of the type string to store vectors is gonna require more memory since each individual char in the string is represented by the ASCII charchters and would have to be saved. 
/// An more logical and better alternative would be to make a object that represent a vector. 