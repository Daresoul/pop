...
///<param name="number">Number to remove all decimals of</param>
///<returns>Returns a float with no deciaml numbers</returns>
...
///<param name="number">The number you wanna create a continued fraction of</param>
...
let main args =
    ...
    let t1 = 3.245
    let r1 = float2cfrac(t1)
    printfn "The float %f has the continued fraction %A expected value %A" t1 r1 [3;4;12;4]

    let t2 = 3.14159265359
    let r2 = float2cfrac(t2)
    printfn "The float %f has the continued fraction %A expected value %A" t2 r2 [3;7;15;1;292]
    // Selvom den continued fraction er længere end forventet synes vi den kom tæt nok på det rigtige resultat, eftersom pi er uendelig

    let t2 = 0.0
    let r2 = float2cfrac(t2)
    printfn "The float %f has the continued fraction %A expected value %A" t2 r2 [0]
    0   